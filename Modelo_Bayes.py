...
model, idata, colnames = ajustar_modelo_bayesiano(df_model)

# Agora `idata` já contém posterior + posterior_predictive
az.to_netcdf(idata, "modules/modelo_bayesiano_trace.nc")

resultados = tabela_resultados(idata, colnames)
resultados.to_csv("modules/resultados_bayes.csv")

# Salvar colnames em CSV (NOVIDADE!)
pd.Series(colnames).to_csv("modules/modelo_bayesiano_colnames.csv", index=False)
