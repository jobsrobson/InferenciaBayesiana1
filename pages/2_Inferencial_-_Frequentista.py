...
if df is not None and not df.empty:
    ...
    def ajustar_modelo(df_model: pd.DataFrame):
        ...
        # Após ajustar o modelo frequentista
        modelo = smf.glm(
            formula=formula,
            data=df_model,
        ...
        with open("modules/modelo_frequentista.pkl", "wb") as f:
            pickle.dump(modelo, f)

        # Também salve os resultados em CSV para comparação
        resultados = pd.DataFrame({
            "Coeficiente (log)": modelo.params,
            "IRR (exp(coef))": np.exp(modelo.params),
            "IC 95% (low)": np.exp(modelo.conf_int()[0]),
            "IC 95% (high)": np.exp(modelo.conf_int()[1]),
            "p-valor": modelo.pvalues
        })
        resultados.to_csv("modules/resultados_freq.csv")
