...
def pagina_frequentista():
    st.markdown('<h1 class="main-header">📐 Modelo Frequentista - RIDE-DF</h1>', unsafe_allow_html=True)
...
def pagina_sobre():
    st.markdown('<h1 class="main-header">ℹ️ Projeto: Análise Completa RIDE-DF</h1>', unsafe_allow_html=True)
    
    st.markdown("""
    ### 🎯 **Projeto Final: Análise Integrada**
    
    **Comparação rigorosa entre abordagens frequentista e bayesiana para predição de taxa de conclusão em cursos superiores da RIDE-DF, usando dados integrados de instituições e estudantes.**
    
    ### 📊 **Arquitetura de Dados Final**
    
    #### 🔗 **Integração sem VIEW (JOIN Inline):**
    ```
    SELECT [100+ colunas]
    FROM ed_superior_ies es
        JOIN municipio_ride_brasilia ride ON ...
        JOIN municipio mun ON ...
        JOIN unidade_federacao uf ON ...
        JOIN regiao reg ON ... 
        LEFT JOIN ed_superior_cursos cur ON (
            es.CO_IES = cur.CO_IES AND 
            es.NU_ANO_CENSO = cur.NU_ANO_CENSO
    ...
    #### 📋 **Estrutura dos Dados:**
    - **~15.000+ registros**: Curso × IES × Ano
    - **82 IES únicas** na RIDE-DF
    - **34 municípios** (DF + GO + MG)
    - **100+ variáveis** integradas
    - **2015-2023**: Série temporal completa
    
    ### 🏛️ **Variáveis Principais**
    
    #### 🎯 **Variável Dependente:**
    ```
    taxa_conclusao = (qt_conc / qt_mat) * 100  # %
    ```
    ...
    |-----------|----------|-------|
    | **🏛️ Institucional** | Categoria admin, % doutores, infraestrutura | IES |
    | **🎓 Acadêmica** | Modalidade, área conhecimento, gratuidade | Cursos |
    | **👥 Demográfica** | % feminino, faixas etárias, raça/cor | Estudantes |
    | **💰 Socioeconômica** | FIES, ProUni, cotas, apoio social | Estudantes |
    | **🗺️ Geográfica** | UF, município, capital/interior, coordenadas | Geografia |
    
    ### 🔬 **Metodologia Científica**
    
    #### ⚖️ **Comparação Frequentista vs Bayesiano:**
    ...
    **Bayesiano:**  
    - Modelos hierárquicos (curso → IES → município → UF)
    - Prioris informativas específicas da RIDE-DF
    - MCMC (PyMC/Stan)
    - Intervalos de credibilidade
    - LOO-CV bayesiano
    
    #### 📈 **Métricas de Avaliação:**
    - **Preditiva**: R², RMSE, MAE
    - **Comparativa**: AIC, BIC, Bayes factors  
    - **Interpretativa**: Significância vs probabilidades
    - **Computacional**: Tempo, escalabilidade
    
    ### 🎓 **Contribuições Esperadas**
    
    #### **🔬 Metodológica:**
    - Framework de comparação frequentista vs bayesiano
    - Benchmarks para dados educacionais hierárquicos
    - Implementação de modelos reproduzíveis
    
    #### **🌍 Regional:**
    - Primeira análise sistemática da RIDE-DF educacional
    - Evidências sobre heterogeneidade DF/GO/MG
    - Insights sobre efeitos metropolitanos na educação
    
    #### **📊 Prática:**
    - Dashboard interativo para gestores
    - Ferramenta de predição para políticas
    - Base para estudos futuros na região
    
    #### **🎯 Acadêmica:**
    - Dissertação/tese em Ciência de Dados
    - Artigos para conferências/revistas
    - Referência para pesquisadores em educação superior
    ...
    - **SQL**: Queries inline otimizadas
    
    #### **📈 Visualização:**
    - **Streamlit**: Dashboard web interativo
    - **Plotly**: Gráficos interativos
    - **Altair**: Visualizações estatísticas
    
    #### **🔢 Modelagem:**
    - **Scikit-learn**: Modelos frequentistas
    - **PyMC/Stan**: Modelos bayesianos
    ...
    |------|---------|-------------|
    | **1. Exploratória** | 2 semanas | Dashboard básico, EDA completa |
    | **2. Frequentista** | 3 semanas | Modelos OLS, validação, métricas |
    | **3. Bayesiano** | 4 semanas | Modelos hierárquicos, MCMC, diagnósticos |
    | **4. Comparação** | 2 semanas | Análise comparativa, resultados |
    | **5. Documentação** | 2 semanas | Relatório final, artigo, apresentação |
    | **6. Deploy** | 1 semana | Dashboard público, código no GitHub |
    ...
    **📊 Total**: ~14 semanas (3.5 meses)
    ...
    #### **📈 Quantitativos:**
    - R² > 0.65 para ambos os modelos
    - RMSE < 15% para taxa de conclusão
    - 95% intervalos informativos
    - Predições para 34 municípios
    
    #### **🧠 Qualitativos:**
    - Compreensão dos drivers de sucesso acadêmico
    - Evidências sobre políticas educacionais efetivas
    - Insights sobre heterogeneidade regional
    - Recomendações para gestão universitária
    
    ### 🎯 **Impacto Esperado**
    
    **🎓 Para Acadêmica:**
    - Metodologia rigorosa e reproduzível
    - Contribuição para literatura de dados educacionais
    - Base para pesquisas futuras na RIDE-DF
    
    **🏛️ Para Gestores:**
    - Dashboard para tomada de decisão
    - Evidências baseadas em dados
    - Ferramenta de planejamento educacional
    
    **💼 Para Carreira:**
    - Portfolio completo de ciência de dados
    - Experiência em modelos estatísticos avançados
    - Competência em análise de políticas públicas
    """)

def main():
    pages = [
        st.Page(pagina_inicio, title="🏠 Análise Integrada", icon="🏠"),
        st.Page(pagina_frequentista, title="📐 Frequentista", icon="📐"),
        st.Page(pagina_bayesiana, title="🎲 Bayesiano", icon="🎲"),
        st.Page(pagina_comparacao, title="⚖️ Comparação", icon="⚖️"),
        st.Page(pagina_sobre, title="ℹ️ Sobre Projeto", icon="ℹ️")
    ]
